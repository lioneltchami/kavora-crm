...
- **Tier-1 + first four Tier-2 items shipped (v1.8):** shadcn/ui sidebar shell + `merge_contacts` PL/pgSQL dedupe (now reconciles loser's `contact_emails` / `contact_phones` into winner) + Sonner `undoable()` soft-delete + DB summary views + mobile list split + multi-value contact channels (data layer + create/edit form UI) + bottom-sheet dialogs + idempotent migration runner with GH Actions auto-apply. The full roadmap (Tier 2 / Tier 3) lives in [`docs/research/atomic-crm/apply-to-kavora.md`](./research/atomic-crm/apply-to-kavora.md) — that doc is the source of truth for what's next.
