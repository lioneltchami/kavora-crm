...
- **What**: Multi-tenant CRM (Phase A shipped). Single org id `"kavora"` is seeded in `organizations`; additional orgs are provisioned via Clerk Organizations webhooks. Every business row carries `orgId` and is scoped by Postgres RLS.
- **Stack**: Next.js 15 (App Router) · Supabase Postgres (Drizzle) · Clerk Organizations · Twilio · Deepgram · Anthropic · Voyage · Trigger.dev · Tailwind + shadcn/ui · Sonner · Vercel.
- **Tenancy**: **multi-tenant** as of Phase A. `KAVORA_ORG_ID` is deprecated; new code uses `currentOrgId()` (server, from Clerk session) or `requireDbUser().orgId`. The DB enforces isolation via RLS keyed on `request.jwt.claims ->> 'org_id'` (resolved through `public.current_org_id()`). Two DB pools: `db` (RLS-firing, default for app code) and `adminDb` (bypasses RLS, for webhooks + background jobs that have no Clerk session).
- **Tier-1 + first four Tier-2 items + Phase A shipped:** shadcn/ui sidebar shell + `merge_contacts` PL/pgSQL dedupe (now reconciles loser's `contact_emails` / `contact_phones` into winner) + Sonner `undoable()` soft-delete + DB summary views + mobile list split + multi-value contact channels (data layer + create/edit form UI) + bottom-sheet dialogs + idempotent migration runner with GH Actions auto-apply + Clerk Organizations + RLS on 18 tables + pgTAP harness + non-superuser app pool + two-pool design (`db` + `adminDb`). The full roadmap (Tier 2 / Tier 3 + the agency-platform pivot) lives in [`docs/research/atomic-crm/apply-to-kavora.md`](./research/atomic-crm/apply-to-kavora.md) and [`docs/research/ai-agency/README.md`](./research/ai-agency/README.md).
...
- **Tenancy seam** — for app code, use `requireDbUser().orgId` (Server Actions, dashboard pages) or `(await auth()).orgId` from `@clerk/nextjs/server` (route handlers with session). For code paths WITHOUT a Clerk session (Clerk webhooks, Twilio inbound routes, Trigger.dev jobs, cron), use `adminDb` from `@/db` instead of `db` — RLS will silently return zero rows otherwise. **Never** use `adminDb` from user-facing request handlers.
...
- ❌ Don't use `db` from `@/db` in webhook routes, Trigger.dev jobs, cron tasks, or any code path with no Clerk session — RLS will silently return zero rows. Use `adminDb` for those paths. The two exports exist precisely for this distinction.
- ❌ Don't use `adminDb` from user-facing request handlers or Server Actions. Admin bypass defeats the multi-tenant isolation guarantee; if you find yourself reaching for adminDb in app code, the right fix is usually to thread the orgId from the Clerk session via `requireDbUser()`.
