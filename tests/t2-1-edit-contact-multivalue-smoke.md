...
| Add a second email/phone row WITHOUT making it primary, then save | **Known limitation (cross-slice follow-up):** the new row is silently dropped — `updateContact` only consumes the legacy `email`/`phone` scalars and uses those to flip an existing primary row (or insert a new primary row only if the legacy scalar value didn't already exist). The `emails`/`phones` JSON arrays in FormData are sent but not yet consumed. Workaround: also flip primary to the new row before saving. Track proper fix in a separate ticket (extend `updateContact` to consume the JSON arrays, OR call `setContactEmailsAndPhones` from the client after `updateContact` succeeds). |
